...
const { getOasFields, getReqFields, getOasResFields } = require('./helpers.general');
...
async function scriptBuilder({ req, allData, connectedSequences }) {
    ...
    try {
        ...
        const reqSequence = connectedSequences[findSequenceIndex(masterNodes[0], masterNodes[1], connectedSequences)]
        ...
        const reqSequenceNodes = reqSequence.flatMap((nodeid) => {
            ...
            node = { ...node };
            node.edges = {
                in: allData.builder.edges.filter((edge) => edge.source == nodeid).filter((edge) => edge.type == "param"),
                out: allData.builder.edges.filter((edge) => edge.target == nodeid).filter((edge) => edge.type == "param")
            };
        ...
        console.log("reqSequenceNodes", reqSequenceNodes)
        ...
        const resSequence = connectedSequences[findSequenceIndex(masterNodes[2], masterNodes[3], connectedSequences)]
        ...
        for (const seq of reqSequence) {
            const nodeIndex = allData.builder.nodes.findIndex(array => array.id === seq)
            let node = allData.builder.nodes[nodeIndex]

            node.edges = allData.builder.edges
                .filter(edge => edge.target.includes(seq))


            const scriptJS = await scripts[node.type];
            const index = masterNodes.findIndex(array => array === node.id)
            script = await scriptJS.build({ allData, index: index, node, variables: reqVariables, params: reqParams, script });
        }
        ...
        for (const seq of resSequence) {
            ...
            script = await scriptJS.build({ allData, index: index, node, variables: resVariables, params: resParams, script });
        }
...
async function grabReqParameters(allData, req) {
    ...
    const { containsParams, typeMatched, parsedParams } = matchTypesAndExtract(fields[0], unexpectedParams);
    if (containsParams && typeMatched) {
        return parsedParams
    } else {
        throw (`Request parameters does not match OAS spec, strict set to ${allData.host.strict_params}`)
    }
}
